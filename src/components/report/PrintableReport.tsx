...
export const PrintableReport: React.FC<PrintableReportProps> = ({ report }) => {
  return (
    <div className="bg-white text-slate-900 mx-auto font-sans print:w-full print:m-0 print:p-0 print:border-none print:shadow-none shadow-lg max-w-[210mm] p-[8mm] border border-slate-200">
      <style jsx global>{`
        @media print {
          @page {
            size: A4 portrait;
      ...
      <div className="page-container flex flex-col justify-between min-h-[270mm] text-[11px] leading-snug">
        {/* TOP SECTION */}
        <div>
          {/* Header */}
          <div className="border-b-2 border-slate-900 pb-2 mb-2 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="text-[13px] font-black tracking-tight uppercase text-slate-950">
                  ŞANTİYE İSG UYGUNSUZLUK VE İHTAR TUTANAĞI
                </h1>
                <p className="text-[9px] text-slate-600 font-medium">
                  6331 Sayılı İSG Kanunu ve Yapı İşlerinde İSG Yönetmeliği Uyarınca Tanzim Edilmiştir
                </p>
              </div>
            </div>
          ...
          {/* Info Summary Strip */}
          <div className="grid grid-cols-2 gap-2 bg-slate-100/90 p-2 rounded-lg border border-slate-300 mb-2 text-[10px]">
            <div>
              <span className="text-slate-500 font-semibold block text-[8px] uppercase">Şantiye / Proje:</span>
              <strong className="text-slate-950 text-[11px] block">{report.siteName}</strong>
            ...
            <div>
              ...
              <strong className="text-slate-950 text-[11px] block">{report.inspectorName}</strong>
          ...
          {/* Hazards List - Full text, no truncation */}
          <div className="space-y-2.5">
            {report.hazards.map((item, index) => (
              ...
              >
                {/* Photo */}
                <div className="w-28 h-26 rounded-md overflow-hidden bg-slate-100 flex-shrink-0 border border-slate-300 relative self-stretch flex items-center justify-center">
                  <img
                    src={item.photoUrl}
                    alt={`Uygunsuzluk #${index + 1}`}
                    className="w-full h-full object-cover"
                  />
                  <span className="absolute bottom-0 inset-x-0 bg-slate-950/85 text-white text-[8px] text-center font-bold py-0.5">
                    {item.severity}
                  </span>
                </div>
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center justify-between gap-2 border-b border-slate-200 pb-0.5">
                    ...
                    <span className="text-rose-700 font-bold text-[9px] bg-rose-50 border border-rose-200 px-1.5 py-0.2 rounded flex-shrink-0">
                      Risk Skoru: {item.riskScore}/25
                    </span>
                  </div>

                  <p className="text-slate-800 text-[9.5px] leading-relaxed">
                    {item.description}
                  </p>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pt-0.5 text-[9px]">
                    <div className="bg-slate-50 p-1.5 rounded border border-slate-200">
                      <strong className="text-slate-600 block text-[8px] uppercase">Yasal Mevzuat Dayanağı:</strong>
                      <span className="text-slate-800 block leading-snug">{item.regulationReference}</span>
                    </div>
                    <div className="bg-amber-50/70 p-1.5 rounded border border-amber-200">
                      <strong className="text-amber-800 block text-[8px] uppercase">DÖF (Gereken Faaliyet):</strong>
                      <span className="text-amber-950 block font-medium leading-snug">{item.correctiveAction}</span>
                    </div>
                  </div>

                  <div className="flex items-center justify-between text-[8.5px] text-slate-600 pt-0.5 border-t border-dashed border-slate-200">
                    <span>Sorumlu Taşeron: <strong className="text-slate-900">{item.subcontractor || 'Ana Yüklenici'}</strong></span>
                    <span>Termin Süresi: <strong className="text-rose-700 font-bold">{item.deadlineHours} Saat</strong></span>
                  </div>
                </div>
              </div>
            ))}
